...
> Companion của `/wiki-trace` (Markdown 1-run) và `/wiki-eval` (Markdown aggregate). HTML self-contained, mở local trong browser.
...
|---|---|
| `/wiki-viz` (không args) | `python {wiki_root}/scripts/render_trace.py --last --project-dir {project_dir}` |
| `/wiki-viz --last` | giống trên |
| `/wiki-viz {run_id}` | `python ... --run {run_id} --project-dir {project_dir}` |
| `/wiki-viz --all` | `python ... --all --project-dir {project_dir}` |
| `/wiki-viz --watch` | `python ... --last --watch --project-dir {project_dir}` |
| `/wiki-viz {run_id} --watch` | `python ... --run {run_id} --watch --project-dir {project_dir}` |
...
UX khuyến nghị: mở 2 terminal — một chạy `/use-wiki "..."`, một chạy `/wiki-viz --watch`. Browser mở sẵn `trace.html`.
...
|---|---|
| Vừa chạy `/use-wiki`, muốn debug 1 run | `/wiki-viz --last` |
| Muốn xem 1 run cũ | `/wiki-viz {run_id}` |
| Muốn so sánh nhiều runs theo workspace/date/verdict | `/wiki-viz --all` |
| Đang chạy `/use-wiki` ở terminal khác, muốn xem live | `/wiki-viz --watch` (terminal mới) |
| Aggregate metrics → text report | dùng `/wiki-eval` thay (Markdown, nhanh hơn) |
| Markdown timeline 1 run trong terminal | dùng `/wiki-trace --last` (không HTML) |
...
- [.claude/commands/wiki-trace.md](wiki-trace.md) — Markdown 1-run viewer (sister command)
- [.claude/commands/wiki-eval.md](wiki-eval.md) — Markdown aggregate (sister command)
