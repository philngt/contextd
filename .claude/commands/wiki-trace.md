...
> Cần HTML viewer (collapsible cards, side-by-side retrieved-vs-used, Mermaid)? Dùng [`/wiki-viz`](wiki-viz.md) — sister command output HTML thay vì Markdown.
...
| `--last` | optional | Thay vì run_id, lấy run mới nhất. `/wiki-trace --last`. |
