...
> Khác `/business-view` (sinh document persistent cho audience cụ thể) — `/wiki-explain` là on-the-fly explainer.
...
- **Khác slash command khác**:
  - `/wiki-report` → HTML technical report toàn workspace (engineering audience)
  - `/business-view` → 1 document persistent dịch 1 service/contract cho audience cụ thể
  - `/wiki-explain` → on-the-fly console explainer cho 1 topic, plain language
