...
Dùng bởi: `code-analyze`, `evidence-ingest`, `evidence-analyze`, `evidence-qa`, `evidence-apply`, `obsidian-ingest`, `contextd-update`, `contextd-use`, `contextd-eval`, `contextd-rebase`.
...
Commands cần dùng effective_packs (không đọc workspace.md trực tiếp): `/evidence-analyze`, `/evidence-qa`, `/contextd-use` planner, mọi pipeline retrieval-map resolution.
