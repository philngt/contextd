...
**Lý do phân tích**: 4 commands explicit follow (code-analyze, evidence-ingest, wiki-setup, new-workspace) — coverage moderate. Có thể nhiều commands khác cũng follow nhưng a03 chỉ enumerate 4. Pattern là UX best practice cho irreversible ops.
...
- `(workspaces/wiki/evidence/sources/2026-05-08-engine-bootstrap-wiki-template/raw.md:L1)` — this evidence follows
...
## q-019 — /wiki-viz orphan README fix: cùng phiên hay tách?  [P1]
...
**Đề xuất câu trả lời**:
> Tách phiên. Apply evidence này trước. Sau đó chạy `/update-wiki --scope .claude/commands/README.md` để add row cho `/wiki-viz` Section "Pipeline observability". KHÔNG tạo ADR (bug fix only).
